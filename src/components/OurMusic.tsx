...
export function OurMusic({ onSectionChange }: OurMusicProps) {
  ...
  // Get today's songs based on date
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-950 via-rose-900 to-black text-white">
      ...
      >
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Disc size={24} className="text-rose-400" />
            <span className="font-bold text-lg">Our Music</span>
          </div>
          <div className="flex items-center gap-2">
            {[
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        ...
        >
          <div>
            <p className="text-rose-300/80 text-sm">{todayDate}</p>
            <p className="text-rose-400/60 text-xs">3 songs for today</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden">
            <motion.div
              className={cn(
                "absolute -inset-20 opacity-30 blur-3xl",
                "bg-gradient-to-r",
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
            {/* Album Art - Clickable to open Spotify */}
            <motion.div
              className={cn(
                "w-48 h-48 md:w-64 md:h-64 rounded-2xl shadow-2xl cursor-pointer group",
                "bg-gradient-to-br",
                currentSong.coverColor,
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <div className="text-center">
                  <ExternalLink size={32} className="text-white mx-auto mb-2" />
                  <p className="text-white text-sm font-medium">Open in Spotify</p>
                </div>
              </div>
            </motion.div>
            ...
            <div className="flex-1 text-center md:text-left">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <p className="text-xl text-rose-200 mb-1">{currentSong.artist}</p>
                  <p className="text-rose-300/60 text-sm mb-6">{currentSong.album}</p>
                </motion.div>
              </AnimatePresence>
              ...
              <div className="mb-6">
                ...
                <div className="flex justify-between text-xs text-rose-300/60 mt-2">
              ...
              <div className="flex items-center justify-center md:justify-start gap-6">
                ...
                {/* Main Play Button - Opens Spotify */}
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                  onClick={openInSpotify}
                  className={cn(
                    "p-5 rounded-full flex items-center gap-2",
                    "bg-green-500 text-black",
                    "hover:bg-green-400 transition-colors shadow-lg shadow-green-500/30"
                  )}
                >
                  <Play size={28} fill="currentColor" className="ml-1" />
                  <span className="font-semibold pr-1">Play on Spotify</span>
                </motion.button>
              ...
              <motion.button
                onClick={openInSpotify}
                className="mt-4 text-green-400 hover:text-green-300 text-sm flex items-center gap-2 mx-auto md:mx-0 transition-colors"
                whileHover={{ x: 3 }}
              >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-rose-300/60 text-sm">{todaysSongs.length} songs</span>
          </div>
          ...
          <div className="space-y-2">
            {todaysSongs.map((song, index) => (
              ...
              >
                ...
                <div
                  className={cn(
                    "w-12 h-12 rounded-lg flex items-center justify-center relative group overflow-hidden",
                    "bg-gradient-to-br",
                    song.coverColor
                  )}
                ...
                >
                  <p className={cn(
                    ...
                    currentSongIndex === index ? "text-rose-300" : "text-white"
                  )}>
                    {song.title}
                  </p>
                  <p className="text-sm text-rose-300/60 truncate">
                    {song.artist}
                  </p>
                </div>
                  ...
                  <Heart
                    ...
                    className={cn(
                      ...
                      likedSongs.has(song.id)
                        ...
                        : "text-rose-300/60 hover:text-rose-400"
                    )}
                  />
                </motion.button>

                {/* Duration */}
                <span className="text-sm text-rose-300/60">
                  {song.duration}
                </span>
        ...
        <motion.div
          ...
          className="mt-8 flex items-center justify-center gap-6 text-rose-300/60"
        >
          <motion.button whileHover={{ scale: 1.1 }} className="p-2 hover:text-white transition-colors">
            <Shuffle size={20} />
          </motion.button>
          <motion.button whileHover={{ scale: 1.1 }} className="p-2 hover:text-white transition-colors">
            <Repeat size={20} />
        ...
        >
          <p className="text-rose-300/80 font-handwritten text-xl">
            &ldquo;Every song is a reminder of how much I love you&rdquo; 💕
          </p>
          <p className="text-rose-400/50 text-sm mt-2">
            Come back tomorrow for 3 new songs
          </p>
        </motion.div>
...
function formatTime(progress: number): string {
  const totalSeconds = Math.floor((progress / 100) * 240); // Assume ~4 min songs
